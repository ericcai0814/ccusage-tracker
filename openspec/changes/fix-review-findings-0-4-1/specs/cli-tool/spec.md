...
- **AND** a single `%` that is not part of a `%NAME%` pair SHALL NOT disqualify the command
- **AND** a command containing `!` anywhere SHALL be treated as third-party
