...
- **WHEN** a command contains `$` or a backtick anywhere, inside double quotes or not (`node "/tmp/$(printf keep)/ccusage-tracker/codex-sync.mjs" --hook`), contains `%` in any form (`%TARGET%`, `%ProgramFiles(x86)%`, `%1%`), or contains a backslash in the script path that is not part of a Windows drive (`C:\…`) or UNC (`\\server\…`) prefix (`node /tmp/ccusage-tracker\codex-sync.mjs --hook`)
